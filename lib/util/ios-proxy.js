...
let iOSProxy = {
  ...
  get options() {
    ...
    switch(Runtime.OS) {
      ...
      case "WINNT":
        // We need to set the PATH someplace where msvcr120.dll is installed
        // and we need to sed CWD to where the rest of the required DLLs are.
        const {OS} = Cu.import("resource://gre/modules/osfile.jsm", {});
        let envService = Cc["@mozilla.org/process/environment;1"].getService(Ci.nsIEnvironment);
        let shell = envService.get("ComSpec") || envService.get("SHELL");
        let path = "PATH=" + envService.get("PATH") + ";" +
                   OS.Constants.Path.libDir + ";" + libPath;
        return {
          env: {
            PATH: path
          }
